...
struct
   ...
   *)
   ...
   let pp_print_char buf c =
     if c = bof then
       Lm_printf.pp_print_string buf "\\bof"
     else if c = eof then
       Lm_printf.pp_print_string buf "\\eof"
     else if c < 32 || c >= 127 then
       Format.fprintf buf "\\%03d" c
     else
       Lm_printf.pp_print_char buf (Char.chr c)
   ...
   let rec pp_print_regex buf regex =
     ...
     | RegexExceptSymbol cl ->
       ...
       Format.fprintf buf ")"
     | RegexLimitPrev cl ->
       Format.fprintf buf "(prev-symbol ";
       IntSet.iter (fun c -> pp_print_char buf c) cl;
       Format.fprintf buf ")"
     | RegexLimitNext cl ->
       Format.fprintf buf "(next-symbol ";
       IntSet.iter (fun c -> pp_print_char buf c) cl;
       Format.fprintf buf ")"
     | RegexChoice el ->
       Format.fprintf buf "@[<hv 3>(choice";
       List.iter (fun e -> Format.fprintf buf "@ %a" pp_print_regex e) el;
       Format.fprintf buf ")@]"
     | RegexSequence el ->
       ...
       Format.fprintf buf ")@]"
     | RegexStar e ->
       Format.fprintf buf "(star %a)" pp_print_regex e
     | RegexPlus e ->
       Format.fprintf buf "(plus %a)" pp_print_regex e
     | RegexInterval (e, min, max) ->
       Format.fprintf buf "@[<hv 3>(interval{%d,%d}@ %a)@]" min max pp_print_regex e
     | RegexArg e ->
       Format.fprintf buf "@[<hv 1>\\(%a\\)@]" pp_print_regex e
   ...
   let regex_of_string s =
     ...
     if !debug_lexgen then
       Format.eprintf "@[<v 3>Regex:@ @[<hv 3>string: \"%s\"@]@ @[<hv 3>regex:@ %a@]@]@." (**)
   ...
   *)
   let pp_print_exp buf exp =
     ...
     in
     Format.fprintf buf "Id: %d" id;
     List.iter (fun (action, id, regex) ->
         Format.fprintf buf "@ @[<hv 3>Clause:@ id = %d@ action = %a@ @[<hv 3>regex =@ %a@]@]" (**)
   ...
   *)
   let pp_print_nfa_id hash buf nid =
     ...
     | (nid, []) ->
       Lm_printf.pp_print_int buf nid
     | (nid, counters) ->
       Format.fprintf buf "<%d" nid;
       List.iter (fun counter -> Format.fprintf buf " %d" counter) counters;
       Format.fprintf buf ">"

   let pp_print_nfa_id_set hash buf states =
     NfaStateSet.iter (fun s ->
         Format.fprintf buf "@ %a" (pp_print_nfa_id hash) s) states

   (* let pp_print_choices buf choices = *)
   (*    IntSet.iter (fun i -> Format.fprintf buf " %d" i) choices *)

   let pp_print_nfa_action buf action =
     match action with
       NfaActionEpsilon next ->
       Format.fprintf buf "@[<hv 3>(epsilon goto";
       List.iter (fun i -> Format.fprintf buf "@ %d" i) next;
       Format.fprintf buf ")@]"
     | NfaActionArgStart (id, next) ->
       Format.fprintf buf "(arg-start %d goto %d)" id next
     | NfaActionArgStop (id, next) ->
       Format.fprintf buf "(arg-stop %d goto %d)" id next
     | NfaActionArgSearch next ->
       Format.fprintf buf "(arg-search goto %d)" next
     | NfaActionStop clause ->
       Format.fprintf buf "(stop [%d])" clause
     | NfaActionSymbol (syms, next) ->
       Format.fprintf buf "@[<hv 3>(symbols [%a]@ goto %d)@]" pp_print_chars syms next
     | NfaActionExceptSymbol (syms, next) ->
       Format.fprintf buf "@[<hv 3>(^symbols [%a]@ goto %d)@]" pp_print_chars syms next
     | NfaActionAnySymbol next ->
       Format.fprintf buf "(. goto %d)" next
     | NfaActionLimitPrev (syms, next) ->
       Format.fprintf buf "@[<hv 3>(limit-prev [%a]@ goto %d)@]" pp_print_chars syms next
     | NfaActionLimitNext (syms, next) ->
       Format.fprintf buf "@[<hv 3>(limit-next [%a]@ goto %d)@]" pp_print_chars syms next
     | NfaActionNone ->
       Format.fprintf buf "(final)"
     | NfaActionResetCounter (counter, next) ->
       Format.fprintf buf "@[<hv 3>(reset@ counter = %d@ goto" counter;
       List.iter (fun i -> Format.fprintf buf "@ %d" i) next;
       Format.fprintf buf ")@]"
     | NfaActionIncrCounter (counter, min, final, max, start) ->
       Format.fprintf buf "@[<hv 3>(increment@ counter = %d@ if count >= %d then goto %d@ if count <= %d then goto %d)@]" (**)
   ...
   let pp_print_nfa_state buf nfa_state =
     ...
     in
     Format.fprintf buf "@[<hv 3>NFA state %d:@ action %a@]" index pp_print_nfa_action action
   ...
   let pp_print_nfa buf nfa =
     ...
     in
     Format.fprintf buf "@[<hv 3>NFA:@ start = %a@ search = %a@ @[<b 3>search-states =%a@]" (**)
       (pp_print_nfa_id hash) start
       (pp_print_nfa_id hash) search
       (pp_print_nfa_id_set hash) search_states;
     Array.iter (fun state ->
         Format.fprintf buf "@ %a" pp_print_nfa_state state) table;
     Format.fprintf buf "@]"
   ...
   *)
   let pp_print_dfa_set hash buf closure =
     Format.fprintf buf "@[<b 3>(set";
     List.iter (fun i -> Format.fprintf buf "@ %a" (pp_print_nfa_id hash) i) closure;
     Format.fprintf buf ")@]"
   ...
   let pp_print_dfa_arg_action buf action =
     ...
     match inst with
       DfaActionArgStart i ->
       Format.fprintf buf "arg-start %d at %d" i off
     | DfaActionArgStop i ->
       Format.fprintf buf "arg-stop %d at %d" i off
     | DfaActionArgSearch ->
       Format.fprintf buf "search-stop at %d" off
   ...
   let pp_print_dfa_actions nfa_hash buf action =
     ...
     in
     let () =
       Format.fprintf buf "@[<hv 3>(action"
     in
     let () =
       match final with
         Some (clause_id, nfa_id) ->
         Format.fprintf buf "@ final [clause=%d, nfa-state=%a]" clause_id (pp_print_nfa_id nfa_hash) nfa_id
       | None ->
         ()
     in
     NfaStateTable.iter (fun dst action ->
         ...
         in
         Format.fprintf buf "@ @[<hv 3>(%a -> %a" (pp_print_nfa_id nfa_hash) src (pp_print_nfa_id nfa_hash) dst;
         List.iter (fun action -> Format.fprintf buf "@ %a" pp_print_dfa_arg_action action) args;
         Format.fprintf buf ")@]") actions;
     Format.fprintf buf ")@]"
   ...
   let pp_print_pre_actions buf action =
     ...
     in
     let () =
       Format.fprintf buf "@[<hv 3>(pre-action@ "
     in
     let () =
       match final with
         Some stop ->
         Format.fprintf buf "final [%d]@ " stop
       | None ->
         ()
     in
     Format.fprintf buf "@[<hv 3>argument actions:";
     List.iter (fun action -> Format.fprintf buf "@ %a" pp_print_dfa_arg_action action) args;
     Format.fprintf buf "@])@]"
   ...
   (*    match trans with *)
   (*       DfaTransition (i, _) -> *)
   (*          Format.fprintf buf "goto %d" i *)
   (*     | DfaNoTransition -> *)
   (*          Format.fprintf buf "error" *)
   (*     | DfaUnknownTransition -> *)
   (*          Format.fprintf buf "unknown" *)

   (* let pp_print_trans_table buf table = *)
   (*    Format.fprintf buf "@[<hv 3>(trans"; *)
   (*    Array.iter (fun (key, trans) -> *)
   (*          Format.fprintf buf "@ %d -> %a" key pp_print_dfa_transition trans) table; *)
   (*    Format.fprintf buf ")@]" *)
   ...
   let pp_print_arg buf arg =
     match arg with
       ArgLeft i ->
       Format.fprintf buf "left[%d]" i
     | ArgRight i ->
       Format.fprintf buf "right[%d]" i
     | ArgSearch ->
       Format.fprintf buf "searchreg"

   (*
    * Print the frontier.
    *)
   let pp_print_frontier nfa_hash buf table =
     Format.fprintf buf "@[<hv 3>frontier:";
     NfaStateTable.iter (fun id actions ->
         Format.fprintf buf "@ @[<hv 3>%a:@ %a@]" (pp_print_nfa_id nfa_hash) id pp_print_pre_actions actions) table;
     Format.fprintf buf "@]"
   ...
   *)
   ...
   let dfa_eval_action dfa info action =
     ...
     if !debug_lex then begin
       ...
       Format.eprintf "@[<v 3>Args:";
       NfaStateTable.iter (fun id args ->
           Format.eprintf "@ @[<hv 3>state: %a" pp_print_nfa_id id;
           ArgTable.iter (fun arg i ->
               Format.eprintf "@ %a = %d" pp_print_arg arg i) args;
           Format.eprintf "@]") args_table;
       Format.eprintf "@]@."
     end;
   ...
   let rec close_prev nfa_hash table nid c closure frontier actions =
     if DfaStateCore.mem closure nid then
       ...
       if !debug_lexgen then
         Format.eprintf "@[<v 3>close_prev:@ NFA state: %a@ Symbol: %a@ @[<hv 3>Closure:@ %a@]@ @[<hv 3>Frontier:@ %a@]@ @[<hv 3>NFA Action:@ %a@]@ @[<hv 3>Actions: %a@]@]@." (**)
   ...
   let rec close_next nfa_hash table nid c closure frontier actions =
     if DfaStateCore.mem closure nid then
       ...
       let action = table.(index).nfa_state_action in
       if !debug_lexgen then
         Format.eprintf "@[<v 3>close_next:@ NFA state: %a@ Symbol: %a@ @[<hv 3>Closure:@ %a@]@ @[<hv 3>Frontier:@ %a@]@ @[<hv 3>NFA Action:@ %a@]@ @[<hv 3>Committed:@ %a@]@]@." (**)
           (pp_print_nfa_id nfa_hash) nid
           pp_print_char c
           (pp_print_dfa_set nfa_hash) closure
           (pp_print_frontier nfa_hash) frontier
       ...
       | NfaActionLimitPrev _ ->
         Format.eprintf "Illegal action: %a@." pp_print_nfa_action action;
   ...
   let close_state dfa table nids c =
     ...
     let final, actions =
       List.fold_left (fun final_actions nid ->
           ...
           in
           if !debug_lexgen then
             Format.eprintf "@[<v 3>Frontier:@ %a@]@." (pp_print_frontier nfa_hash) frontier;
     ...
     let () =
       if !debug_lexgen then
         ...
         in
         Format.eprintf "@[<v 3>Computed actions:%a@]@." (pp_print_dfa_actions nfa_hash) actions
     in
   ...
   let close_next_state dfa table nids c =
     ...
     if !debug_lex then
       Format.eprintf "@[<hv 3>NFA transition:@ @[<hv 3>current:@ %a@]@ symbol: %a@ @[<hv 3>next:@ %a@]@ @[<hv 3>actions:@ %a@]@." (**)
   ...
   let rec dfa_delta dfa dfa_info dfa_state c =
     match TransTable.find dfa_state.dfa_state_delta c DfaUnknownTransition with
       ...
       if !debug_lex then
         Format.eprintf "State %d %a: symbol %a, goto %d@." (**)
     ...
     | DfaNoTransition ->
       if !debug_lex then
         Format.eprintf "State %d %a: no transition for symbol %a@." (**)
           dfa_state.dfa_state_index
           (pp_print_dfa_set dfa.dfa_nfa_hash) (DfaState.get dfa.dfa_dfa_hash dfa_state.dfa_state_set)
           pp_print_char c;
       None
     | DfaUnknownTransition ->
       if !debug_lex then
         Format.eprintf "State %d %a: computing transition on symbol %a@." (**)
           dfa_state.dfa_state_index
           (pp_print_dfa_set dfa.dfa_nfa_hash) (DfaState.get dfa.dfa_dfa_hash dfa_state.dfa_state_set)
           pp_print_char c;
   ...
   let create exp =
     ...
     let () =
       if !debug_lexgen || !debug_lex then
         Format.eprintf "%a@." pp_print_nfa nfa
     in
   ...
   let pp_print_lexer buf info =
     ...
     Format.fprintf buf "@[<v 0>@[<hv 3>Lexer:@ %a@]" pp_print_exp exp;
     Format.fprintf buf "@ @[<hv 3>NFA:";
     Array.iter (fun nfa_state -> Format.fprintf buf "@ %a" pp_print_nfa_state nfa_state) dfa.dfa_table;
     Format.fprintf buf "@]@]"
...
struct
   ...
   let pp_print_action = Lm_printf.pp_print_int
