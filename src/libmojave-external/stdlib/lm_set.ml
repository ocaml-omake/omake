...
module LmMake (Ord : Lm_set_sig.OrderedType) =
struct
  (************************************************************************
   * TYPES                                                                *
   ************************************************************************)
      ...
      begin
        ...
        if comp = 0 then
          ...
          | Red (key1, left1, right1, size1) ->
            ...
            if comp = 0 then raise Unchanged
            else if comp < 0 then
  ...
  let rec mem t key =
    ...
    | Red (key', left, right, _) ->
      let comp = Ord.compare key key' in
      if comp = 0 then true
      else if comp < 0 then mem left key
      else mem right key
    | Leaf ->
      false

  (** added new elements to match standard library *)
  let rec find key set = 
    match set with 
    ...
    | Red(key', left,right, _) -> 
      let comp = Ord.compare key key' in
      if comp = 0 then key'
      else if comp < 0 then find key  left 
      else find key  right 
  ;;
  ...
  (*   | Red(key',left,right) ->  *)
  (*     let comp = Ord.compare key key' in *)
  (*     if comp = 0 then (left, true, right) *)
  (*     else if comp < 0 then  *)
  (*       let (ll, pres, rl) = split key left in  *)
  (*       (ll, ) *)
  (*     else  *)
...
module LmMakeDebug (Ord : Lm_set_sig.OrderedTypeDebug) =
struct
  include LmMake (Ord)
